...
namespace SMS_Project.Controllers
{
    ...
    public class mentality_inquiriesController : ControllerBase
    {
        ...
        public List<mentality_inquiries> Get()
        {
            ...
            var convertedList = (from rw in dataset1.Tables[0].AsEnumerable()
                                 select new mentality_inquiries()
                                 {
                                     ...
                                     problem_type = Convert.ToString(rw["status_name"]),
                                     answer = Convert.ToString(rw["status_id"]),
                                     ntoes = Convert.ToString(rw["level_id"])
        ...
        // POST api/<controller>
        [HttpPost]
        public void Post(mentality_inquiries obj)
        {
